...
typedef struct {
  ...
  XftColor win[ColLast];      //rename func patch
	Drawable drawable;
	GC gc;
	struct {
		int ascent;
...
typedef struct {
	char name[256];
  char title[256];    //rename func patch
	Bool titleset;      //rename func patch
	Window win;
	int tabx;
	Bool urgent;
	Bool closed;
...
static void renametab(const Arg *arg);                           //rename func patch 
static void settabtitle(const char *title, unsigned int size);   //rename func patch
static void unsettabtitle(const char *title, unsigned int size); //rename func patch
